...
func TestSmtpConfigureEmptyHost(t *testing.T) {
	...
	if smtpConfigured {
		t.Errorf("SMTP configured when it should not be due to empty SAMLETCHAT_SMTP_HOST")
		return
	}
}
...
func TestSmtpConfigureEmptyAddress(t *testing.T) {
	...
	if err := smtpConfigure(); err == nil {
		t.Errorf("expected error not found; SMTP should not be configured when SAMLETCHAT_SMTP_FROM_ADDRESS is empty")
		return
	}
}
