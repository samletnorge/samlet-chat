...
On pages where you want to display comment counts, append a `#samlet-chat` to the end of the `href` attribute of links should display the number of comments. Commento will automatically look for such links and fill them with the number of comments. For example, consider the following link.

<pre class="gray">
&lt;a href="https://example.com/foo<b><code class="bold">#samlet-chat</code></b>"&gt;&lt;/a&gt;
</pre>

When the `count.js` snippet is included, Commento would automatically search for all such links ending with `#samlet-chat` and fill the inner text with the number of comments, like so:
