### samlet-chat Single Sign-On

Single sign-on (SSO) is a custom authentication protocol that delegates user management to a different service. For example, if you already support user registration and login, you may want your readers to log in using your service instead of creating yet another account. samlet-chat's SSO lets you do that.

#### Enabling SSO

To enable SSO in samlet-chat, go to the dashboard and tick the corresponding box in your domain's general settings, like so:
...
#### Implementing samlet-chat SSO

Integrating samlet-chat's SSO protocol with your website requires some backend work.

Let's say you have samlet-chat embedded in your blog `blog.example.com`. When the user tries to log in with SSO, samlet-chat will redirect users to the redirect URL you set in the dashboard. For the rest of this document, we'll assume this is set to `http://example.com/sso/samlet-chat`. There can only be one redirect endpoint for each domain.
...
Using the shared secret key, you must generate the HMAC-SHA256 for the JSON payload described above. Following this, hex-encode the payload and the HMAC; let's call the hex-encoded strings `PAYLOAD` and `HMAC`, respectively. Redirect the user to samlet-chat's SSO callback:

```
http://samlet-chat.valiantlynx.com/api/oauth/sso/callback?payload=PAYLOAD&hmac=HMAC
```

samlet-chat will verify the HMAC-SHA256 to authenticate the callback and log the corresponding user in. A new samlet-chat account will be automatically created if this is the first time the user is logging in this way.
...
1. Your HMAC secret key is kept securely. If you lose this, anybody can impersonate requests from samlet-chat and you will leak personal data.
...
1. You're authenticating your users properly before sending samlet-chat a response payload. This includes email verification, two-factor authentication, access control lists, and so on.
1. You're redirecting to the HTTPS version of samlet-chat at the end. Personal data is transmitted in this stage and using plain HTTP is grossly insecure.
